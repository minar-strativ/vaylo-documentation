...
2026-08-18 | cancellation-rules | needs_review | CancellationRuleGroup/CancellationRule model + FKs on Tour/TourType/BookingCancellationGroup are model-only scaffolding -- no service/view/serializer reads or computes from them yet; no doc written, same treatment as rewards | 5609d882b
2026-08-18 | crm | needs_review | confirmed during this run: crm/services/__init__.py imports a CRMLogService module that does not exist (broken import); model-only scaffolding, no doc written | 5609d882b
2026-08-18 | maintain run completed | 20 features re-assessed (1 real update: dynamic-forms; 19 re-anchored, no content change) + 3 new candidates from 58 unclaimed paths: tour-flights and shuttle-timetable documented as new completed features (cross-referenced from booking/tours/flights-tickets/billing/notifications/notes-todos), cancellation-rules and crm flagged needs_review as unreleased model-only scaffolding. progress.json reset to idle. | 5609d882b
